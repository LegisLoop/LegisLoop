...
import {
    ...
    ScaleIcon,
    ShieldIcon,
    IdentificationIcon, LockIcon, HomeIcon, BuildingBridgeIcon, InformationCircleIcon
} from "../Icons/Icons";
...
function SideBar() {
    ...
    const handlePolicyClick = (policy) => {
        setActivePolicy(policy === activePolicy ? null : policy);
    };
                        ...
                        <div className={`overflow-hidden duration-300 ease-in-out ${isPolicyOpen ? 'max-h-100 opacity-100' : 'max-h-0 opacity-0'}`}>
                            <div
                                ...
                                <nav
                                    className="flex min-w-[240px] flex-col gap-1 p-0 font-sans text-base font-normal text-blue-gray-700">
                                    {[
                                        { icon: <HeartIcon/>, label: "Public Health" },
                                        { icon: <CurrencyDollarIcon/>, label: "Economic Policy" },
                                        { icon: <BookOpenIcon/>, label: "Education Policy" },
                                        { icon: <GlobalAmericasIcon/>, label: "Environmental Policy" },
                                        { icon: <HandRaisedIcon/>, label: "Religious Policy" },
                                        { icon: <SparklesIcon/>, label: "LGBTQ+ Rights" },
                                        { icon: <ScaleIcon/>, label: "Civil Rights & Liberties" },
                                        { icon: <ShieldIcon/>, label: "Law & Public Safety" },
                                        { icon: <IdentificationIcon/>, label: "Immigration Policy" },
                                        { icon: <LockIcon/>, label: "Data & Privacy Laws" },
                                        ...
                                        <div key={index} role="button"
                                             ...
                                             className={`flex items-center w-full p-3 leading-tight transition-all rounded-lg outline-none text-start ${
                                                 activePolicy === item.label ? "bg-custom-red-light bg-opacity-50 border-l-4 border-custom-red text-custom-red-dark font-semibold" : "hover:bg-gray-200"
                                             }`}>
                                            <div className={`grid mr-4 place-items-center ${
                                                activePolicy === item.label ? "text-custom-red-dark" : "text-custom-blue"
                                            }`}>
                                                {item.icon}
                                            </div>
                                            {item.label}
